#include "PrescripteurListModel.h"
...
PrescripteurListModel::PrescripteurListModel(QObject *parent): QAbstractListModel(parent)
{

}

QVariant PrescripteurListModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
    ...
    Prescripteur item = list.at(index.row());
    switch (role) {
    case Qt::DisplayRole :
        break;
    case ID_ROLE:
...
int PrescripteurListModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
    return list.size();
...
QHash<int, QByteArray> PrescripteurListModel::roleNames() const
{
