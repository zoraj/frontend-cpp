#include "SegmentationListModel.h"
...
SegmentationListModel::SegmentationListModel(QObject *parent): QAbstractListModel(parent)
{

}

QVariant SegmentationListModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
    ...
    Segmentation item = list.at(index.row());
    switch (role) {
    case Qt::DisplayRole :
        break;
    case ID_ROLE:
...
int SegmentationListModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
    return list.size();
...
QHash<int, QByteArray> SegmentationListModel::roleNames() const
{
