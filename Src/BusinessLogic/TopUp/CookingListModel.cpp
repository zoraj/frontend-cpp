...
CookingListModel::CookingListModel(QObject * parent): QAbstractListModel(parent)
{

}

QVariant CookingListModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
...
int CookingListModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
    return list.size();
...
QHash<int, QByteArray> CookingListModel::roleNames() const
{
