#include "AccompanimentListModel.h"
...
AccompanimentListModel::AccompanimentListModel(QObject * parent): QAbstractListModel(parent)
{

}

QVariant AccompanimentListModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
...
int AccompanimentListModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
    return list.size();
...
QHash<int, QByteArray> AccompanimentListModel::roleNames() const
{
