#include "CashingModeListModel.h"
...
CashingModeListModel::CashingModeListModel(QObject * parent): QAbstractListModel(parent)
{

}

QVariant CashingModeListModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
...
int CashingModeListModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid())
        return 0;
    return list.size();
...
QHash<int, QByteArray> CashingModeListModel::roleNames() const
{
